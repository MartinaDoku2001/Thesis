...
The transformer has been built using two main entieties the encoder
and the dceoder. Th einput is fed to the encoder while the decoder takes 
...
dependencies between words, and long-range dependencies. Firstly, 
...
connections and batch normalization, known as BasicBlockBiGru, to capture 
...
to capture the dependencies between different words.
...
attention and FFN layers.
\item BasicBlockBiGru: A sequence of bidirectional GRU layers with 
...
network that processes the output of the attention mechanism.
\end{itemize}
...
decoder input to capture dependencies between different words in the 
...
feed-forward network. Layer normalization is performed on the outputs of the attention and feed-forward network layers, and dropout regularization is applied. BasicBlockBiGru, consisting of bidirectional GRU layers with residual connections and batch normalization, is utilized to enhance the model's performance. The Decoder takes the inputs and encoder outputs, performing the aforementioned operations, and returns the final output of the decoder. The Decoder class consists of the following components:
Here it's provided a deeper explanation of the components of the decoder:
\begin{itemize}
  \item MultiHeadAttention (self-attention): Performs self-attention on 
  the decoder input to capture dependencies between different words in the output sequence.
  \item MultiHeadAttention (encoder-decoder attention): Computes attention 
  between the decoder input and the output of the encoder to incorporate the encoded information.
  \item Feed-Forward Network (FFN): Processes the output of the attention 
  mechanisms.
  \item Positional Embedding: Embeds the positions of the decoder input 
  elements to capture positional information.
  \item Layer Normalization: Normalizes the outputs of the attention and 
  FFN layers.
  \item Dropout: Applies dropout regularization to the output of the 
  attention and FFN layers.
  \item BasicBlockBiGru: A sequence of bidirectional GRU layers with 
...
Each resBiGru layer consists of three bidirectional Gated Recurrent 
...
its ability to understand the intricate relationships between words. \newline \\
...
subsequent to the batch normalization step. This activation function 
...
complex patterns, the resBiGru layer is repeated three times. This 
...
context-specific information. Importantly, the same set of operations, 
namely bidirectional GRU layers, batch normalization, and ReLU 
activation, are consistently applied in each repetition, 
ensuring consistency and coherence in the model's behavior.
